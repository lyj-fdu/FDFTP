...
- 发现超时和接收窗口非常影响性能。超时重传过慢会无法及时利用带宽，但过快会造成太多冗余包；窗口过小会无法充分利用带宽，过大的话如果网络拥塞丢失大片包时TCP-NewReno的快速恢复会造成停等性能降低。过去，固定超时重传为1s，窗口大小设为os.write的写入限制太小，因此现作改进：
  - 初始连接时通过ping确定超时重传时间，ping失败才设为默认重传时间`DEFAULT_CONG_TIMEOUT`，且默认重传时间设为0.1s，因为普通网络延迟在1-100ms
  - `RWND`可设置任意大，不再受os.write写入大小的限制，实质上是分多次写入。从网上搜多的研究表明，对于 65,535字节的接收窗口（`RWND`=43），在 RTT 为100ms的路径上只能达到速度大约5.24Mbps的吞吐量，而不管传输路径的实际带宽是多少。助教提供的服务器吞吐量最大为30Mbps，但延迟在100-300ms波动，因此暂且`RWND`=200左右比较合适。
- 发现如果客户端一次能串行传多个文件无法解决的问题。
  - 发送端发fin，接收端确认finack表示收到文件
  - 如果fin丢了，发送端会超时重传，直到接收端确认，这能保证接收方收到文件
  - 但一旦接收方收到文件，只发送一次finack，不能保证发送方一定收到，这时发送方还会一直传包，而接收方已经到了传下一个文件阶段，收到上次传文件阶段发送的包就会发finack，直到发送方接收finack退出传输
  - 如果发送方启动下一次传输，可能收到上次的finack直接结束传输，因此无法串行传多个文件（或需要更复杂的机制？暂时没想到……）
  - 因此，把客户端改为一次上传或下载一个文件。主流的文件上传下载服务也是这样的。不必模仿TCP的持续传输。毕竟这是FTP。
